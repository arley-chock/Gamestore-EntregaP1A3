...
const Home = () => {
  ...
  const carregarCategorias = async () => {
    ...
    try {
      const dadosCategorias = await apiFetch('/categorias', {
        auth: true
      })
  ...
  const carregarJogos = async (categoriasMap) => {
    try {
      const dadosJogos = await fetchGames()
      ...
      const jogosNormalizados = dadosJogos.map((jogo, index) => {
        const fkCategoria = jogo.fkCategoria || jogo.fk_categoria;
        
        return {
          ...jogo,
          id: jogo.id || jogo.ID || jogo.id_jogo || `public-${index}`,
          fkEmpresa: jogo.fkEmpresa || jogo.fk_empresa,
          fkCategoria: fkCategoria,
          categoria: categoriasMap[fkCategoria] || jogo.categoria || 'Desconhecida',
          preco: Number(jogo.preco) || 0,
          ano: typeof jogo.ano === 'string' ? parseInt(jogo.ano.split('-')[0] || jogo.ano, 10) || 0 : (jogo.ano || 0)
        }
      })
      ...
      if (!categoriasMap || Object.keys(categoriasMap).length === 0) {
        const categoriasDerivadas = [...new Set(jogosNormalizados.map(j => j.categoria).filter(Boolean))]
        setCategorias(categoriasDerivadas.map((nome, idx) => ({ id: `local-${idx}`, nome })))
      }
