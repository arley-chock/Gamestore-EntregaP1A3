...
const Suporte = () => {
  ...
  const carregarMeusChamados = useCallback((currentUser) => {
  ...
  useEffect(() => {
    ...
    setLoading(false);
    carregarMeusChamados(user);
  }, [authLoading, user, carregarMeusChamados, navigate]);
  ...
  const handleSubmit = (e) => {
    ...
    const novoChamado = {
      usuarioId: user.id || user.usuarioId || null,
      nome: user.nome || '',
      email: user.email || '',
      assunto,
      mensagem,
    ...
    setAssunto('');
    setMensagem('');
    carregarMeusChamados(user); // Atualiza a lista de chamados na interface
  };
